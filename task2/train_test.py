...
# 读取一批captcha作为样本
def get_next_batch(start, batch_size=200):
	batch_x = np.zeros([batch_size, IMAGE_HEIGHT * IMAGE_WIDTH])
	batch_y = np.zeros([batch_size, MAX_CAPTCHA * CHAR_SET_LEN])

	#def wrap_get_captcha_text_and_image():
		#while True:
		#text, image = get_captcha_text_and_image(linee, img_name)
		#if image.shape == (IMAGE_HEIGHT, IMAGE_WIDTH, 3):
		#linee += 1
		#return text, image
	line = start
	img_name = 'train/' + str('%04d' % line) + '.jpg'

	for i in range(batch_size):
		text, image = get_captcha_text_and_image(line, img_name) #wrap_get_captcha_text_and_image()
...
def train_crack_captcha_cnn():
	...
	with tf.Session() as sess:
		...
		st = 0
		while True:
			batch_x, batch_y = get_next_batch(st, 64)
			st += 1
